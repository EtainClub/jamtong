...
실린 내용이 사실과 다르면 `/correction`으로 접수받는다. 업적과 언행 화면
끝에서 그 주소로 들어오고, 어느 화면이었는지가 링크에 따라온다.

피드백(`/feedback`)과 나란히 두되 섞지 않는다. 피드백은 사이트에 바라는
것이고 **공개가 기능**이다. 정정·반론은 실린 내용이 틀렸다는 주장이라
운영자에게만 간다 — 공개 게시판에 두면 접수함이 아니라 또 하나의 발행면이
된다. 접수한 글은 Firestore `corrections`에 쌓이고, 본인과 운영자만 읽는다
(`firestore.rules`, `pnpm check:rules`가 실제로 막히는지 대 본다).
